...
+ (NSString *)getFriendlyDateString:(NSTimeInterval)timeInterval {
    return [JMSGStringUtils getFriendlyDateString:timeInterval forConversation:NO];
}
...
+ (NSString *)getFriendlyDateString:(NSTimeInterval)timeInterval
    ...
    if (theDiff < 60) {
        ...
        if (isTodayYesterday) {
            NSString *todayYesterday = [JMSGStringUtils getTodayYesterdayString:theDate];
            if (isShort) {
            ...
            output = [formatter stringFromDate:theDate];
            if (isPastLong) {
                NSString *thePastDate = [JMSGStringUtils getPastDateString:theDate];
