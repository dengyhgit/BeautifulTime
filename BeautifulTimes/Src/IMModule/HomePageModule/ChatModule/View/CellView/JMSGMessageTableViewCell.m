...
#define messagePercentLabelFrame [_model.message isReceived]?CGPointMake(_messageContent.frame.size.width/2 + 10, _messageContent.frame.size.height/2):CGPointMake(_messageContent.frame.size.width/2 - 10, _messageContent.frame.size.height/2)
...
@implementation JMSGMessageTableViewCell
...
- (id)initWithStyle:(UITableViewCellStyle)style
    ...
    if (self) {
        ...
        _messageContent = [JMSGMessageContentView new];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 1) {
        ...
        if (_model.message.contentType == kJMSGContentTypeImage) {
            ...
            [[JMSGSendMsgManager ins] addMessage:weakSelf.model.message withConversation:_conversation];
        } else {
        ...
        if (error == nil) {
            if (data != nil) {
                ...
                if (!_isPlaying) {
                    ...
                    [[JMSGAudioPlayerHelper shareInstance] setDelegate:(id) self];
                    _isPlaying = YES;
                } else {
                    _isPlaying = NO;
                    self.continuePlayer = NO;
                ...
                [[JMSGAudioPlayerHelper shareInstance] managerAudioWithData:data toplay:YES];
